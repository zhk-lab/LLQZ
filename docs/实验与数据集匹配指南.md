...
- 示例：
  - 快速冒烟（先跑 30 题）：
    - `python scripts/exp2_run_pipeline.py --model <你的模型名> --m 10 --max_items 30 --require_same_label --resume`
  - 全量运行：
    - `python scripts/exp2_run_pipeline.py --model <你的模型名> --m 10 --require_same_label --resume`
- 提速配置（目标 5～6 小时，保持 `m=10`）：
  - `python scripts/exp2_run_pipeline.py --model <你的模型名> --m 10 --workers 4 --max_context_chars 2500 --max_tokens 160 --concise_mode --append_no_think --require_same_label --resume`
