...
- 一键流水线：`scripts/exp1_run_pipeline.py`
 ...
 - **评估建议（与实验三保持一致）**：
   - 建议用 `top_k=3` 复算检索指标，能更好体现 `hybrid_rerank` 的前列排序优势：
   - `python scripts/exp1_evaluate_and_plot.py --pred_root runs/exp1 --strategies sparse_only,dense_only,hybrid,hybrid_rerank --top_k 3 --metrics_out runs/exp1/metrics_k3_all_strategies.json --plot_metrics_out plots/exp1_retrieval_metrics_k3_all_strategies.png --plot_cost_out plots/exp1_effect_vs_cost_k3_all_strategies.png`
   - 若要与旧口径对照，可保留 `top_k=20` 指标作为补充材料。
